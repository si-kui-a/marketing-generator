...
| 零高風險自動化 | 只保留寫死不變、無需持續除錯的固定腳本 |
| 受控自動化例外 | GitHub Contents API 讀寫為架構必要,不屬於高風險自動化 |
...
| 方法 | 路徑 | 功能 |
|---|---|---|
| GET | `/brands` | 掃描 `data/brand/` 檔名,回傳品牌清單,經 GitHub Contents API 讀寫 |
| GET | `/brand/<name>` | 回傳單一品牌 md 內容,經 GitHub Contents API 讀寫 |
| POST | `/generate` | 組合 brand + sample + prompt 模板,呼叫 provider,回傳 N 版文案,經 GitHub Contents API 讀寫 |
| POST | `/archive` | 寫入 `data/archive/YYYY/日期-品牌-類型.md`,含 YAML frontmatter,經 GitHub Contents API 讀寫 |
| POST | `/tag` | 標記 archive 的 `performance_tag`,同步累加 `tag_frequency.json`,經 GitHub Contents API 讀寫 |
| POST | `/samples/batch` | 批次貼入多篇文案,關鍵字比對後回傳預標記(未落地),經 GitHub Contents API 讀寫 |
| POST | `/samples/confirm` | 人工確認後正式寫入 `data/sample/`,觸發自動備份,經 GitHub Contents API 讀寫 |
